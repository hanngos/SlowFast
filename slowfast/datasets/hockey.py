...
class Hockey(torch.utils.data.Dataset):
    """
    Hockey video loader. Construct the Hockey video loader, then sample
    ...
    def __init__(self, cfg, mode, num_retries=100):
        """
        Construct the Hockey video loader with a given csv file. The format of
        the csv file is:
        ```
        ...
        assert mode in [
            ...
            "val",
            "test",
        ], "Split '{}' not supported for Hockey".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        logger.info("Constructing Hockey {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        ), "Failed to load Hockey split {} from {}".format(
            self._split_idx, path_to_file
        )
        logger.info(
            "Constructing Hockey dataloader (size: {} skip_rows {}) from {} ".format(
