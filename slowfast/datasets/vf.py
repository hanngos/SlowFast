...
class Vf(torch.utils.data.Dataset):
    """
    VF video loader. Construct the VF video loader, then sample
    ...
    def __init__(self, cfg, mode, num_retries=100):
        """
        Construct the VF video loader with a given csv file. The format of
        the csv file is:
        ```
        ...
        assert mode in [
            ...
            "val",
            "test",
        ], "Split '{}' not supported for VF".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        logger.info("Constructing VF {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        ), "Failed to load VF split {} from {}".format(
            self._split_idx, path_to_file
        )
        logger.info(
            "Constructing VF dataloader (size: {} skip_rows {}) from {} ".format(
